...
Slugs are generated by `Noerd\Cms\Services\PageSlugService` and kept unique per tenant and language at the application level — `slug` is a JSON language map, so there is no database unique index for it. Slugs are auto-generated from the page name. For non-default languages, a language prefix is added automatically:
