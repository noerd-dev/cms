...
new class () extends Component {
    ...
    public function mount(Page $page, ?string $collectionKey = null): void
    {
        ...
        // Note: YAML fields use wire:model="pageData.xxx" so collection data must be in pageData
        if ($this->collectionKey && isset($this->pageData['data']) && is_array($this->pageData['data'])) {
            // Store collection fields in collectionData for reference
    ...
    private function getCollectionFieldNames(): array
    {
        ...
        foreach ($this->collectionLayout['fields'] as $field) {
            $name = $field['name'] ?? '';
            // Strip pageData. or model. prefix to get the actual field name
    ...
    private function storeCollectionPage(): void
    {
        ...
        // Extract only collection-specific fields from pageData (not name, slug, etc.)
        $rawCollectionData = $this->extractCollectionData();
        // Apply field type conversion to collection data
        ...
        $data = [
            ...
            'data' => $convertedCollectionData,  // Only collection-specific fields
        ...
        if ($hasPageFeatures) {
            // Name: Use directly from pageData
            $nameData = [];
            ...
            // Slug: Use existing from pageData, only auto-generate if empty
            $slugData = [];
            if (isset($this->pageData['slug']) && is_array($this->pageData['slug'])) {
                foreach ($this->pageData['slug'] as $lang => $slug) {
                    if (!empty($slug)) {
                        // Use the manually entered/existing slug
                        $slugData[$lang] = $slug;
