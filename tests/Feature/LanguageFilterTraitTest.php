...
it('resolves the default, active and selected language codes from the tenant', function (): void {
    $this->actingAsCmsUser();
    $this->useOnlyLanguage('da', 'Dansk');
    ...
    $probe = new class {
        ...
        {
            return [$this->defaultLanguageCode(), $this->activeLanguageCodes(), $this->selectedLanguageCode()];
        }
    };

    session(['selectedLanguage' => 'en']);
    expect($probe->codes())->toBe(['da', ['da', 'en'], 'en']);

    // A stale selection that is no longer active falls back to the default.
    session(['selectedLanguage' => 'fr']);
    expect($probe->codes()[2])->toBe('da');
});
