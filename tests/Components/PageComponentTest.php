...
it('stores slug in dedicated column not in data column for regular pages', function () use ($testSettings): void {
    ['user' => $user, 'tenant' => $tenant] = $this->createUserWithCmsAccess();
    ...
    Volt::test($testSettings['componentName'])
        ->set('pageData.name.de', 'Test Seite')
        ->set('pageData.name.en', 'Test Page')
        ->set('pageData.slug.de', '/mein-custom-slug')
        ->set('pageData.slug.en', '/my-custom-slug')
        ->set('pageData.layout', 'weblayout')
        ->call('store')
        ->assertOk();
    ...
    expect($page->slug)->toBeArray()
        ->and($page->slug['de'])->toBe('/mein-custom-slug')
        ->and($page->slug['en'])->toBe('/my-custom-slug');

    // Slug should NOT be in the data column
    expect($page->data)->toBeNull();
});

it('stores user-entered slug instead of auto-generated for regular pages', function () use ($testSettings): void {
    ['user' => $user, 'tenant' => $tenant] = $this->createUserWithCmsAccess();

    $this->actingAs($user);

    // Create page with custom slug that differs from auto-generated
    Volt::test($testSettings['componentName'])
        ->set('pageData.name.de', 'Meine Test Seite')
        ->set('pageData.name.en', 'My Test Page')
        ->set('pageData.slug.de', '/komplett-anderer-slug')
        ->set('pageData.slug.en', '/completely-different-slug')
        ->set('pageData.layout', 'weblayout')
        ->call('store')
        ->assertOk();
    ...
    expect($page->slug['de'])->toBe('/komplett-anderer-slug')
        ->and($page->slug['en'])->toBe('/completely-different-slug');
});
