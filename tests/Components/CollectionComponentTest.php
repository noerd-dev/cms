...
it('stores slug in dedicated column not in data column for collection pages', function () use ($testSettings): void {
    ['user' => $user, 'tenant' => $tenant] = $this->createUserWithCmsAccess();
    $this->actingAs($user);

    // Create a collection for contacts (hasPage: true)
    $parentCollection = Collection::factory()->create([
        'tenant_id' => $tenant->id,
        'collection_key' => 'CONTACTS',
    ]);

    // Test the page-component with contacts collection key
    Volt::test($testSettings['componentName'], ['collectionKey' => 'contacts'])
        ->set('pageData.name', ['de' => 'Test Kontakt', 'en' => 'Test Contact'])
        ->set('pageData.slug', ['de' => '/mein-kontakt-slug', 'en' => '/my-contact-slug'])
        ->call('store')
        ->assertHasNoErrors();

    $page = Page::where('collection_id', $parentCollection->id)->latest('id')->first();

    // Slug should be in the dedicated column
    expect($page->slug)->toBeArray()
        ->and($page->slug['de'])->toBe('/mein-kontakt-slug')
        ->and($page->slug['en'])->toBe('/my-contact-slug');

    // Slug should NOT be in the data column
    expect($page->data)->not->toHaveKey('slug');
});

it('stores user-entered slug instead of auto-generated for collection pages', function () use ($testSettings): void {
    ['user' => $user, 'tenant' => $tenant] = $this->createUserWithCmsAccess();
    $this->actingAs($user);
    ...
    // Create collection page with custom slug that differs from auto-generated
    Volt::test($testSettings['componentName'], ['collectionKey' => 'contacts'])
        ->set('pageData.name', ['de' => 'Max Mustermann', 'en' => 'John Doe'])
        ->set('pageData.slug', ['de' => '/komplett-anderer-kontakt-slug', 'en' => '/completely-different-contact-slug'])
        ->call('store')
        ->assertHasNoErrors();

    $page = Page::where('collection_id', $parentCollection->id)->latest('id')->first();

    // Should use user-entered slug, not auto-generated from name
    expect($page->slug['de'])->toBe('/komplett-anderer-kontakt-slug')
        ->and($page->slug['en'])->toBe('/completely-different-contact-slug');
});

it('does not store name and layout in data column for collection pages', function () use ($testSettings): void {
    ['user' => $user, 'tenant' => $tenant] = $this->createUserWithCmsAccess();
    $this->actingAs($user);
    ...
    Volt::test($testSettings['componentName'], ['collectionKey' => 'contacts'])
        ->set('pageData.name', ['de' => 'Test Kontakt', 'en' => 'Test Contact'])
        ->set('pageData.slug', ['de' => '/test-kontakt', 'en' => '/test-contact'])
    ...
    // Name should be in dedicated column
    expect($page->name)->toBeArray()
        ->and($page->name['de'])->toBe('Test Kontakt');

    // Layout should be in dedicated column
    expect($page->layout)->toBe('weblayout');

    // These should NOT be in the data column
    expect($page->data)->not->toHaveKey('name')
        ->and($page->data)->not->toHaveKey('slug')
        ->and($page->data)->not->toHaveKey('layout')
        ->and($page->data)->not->toHaveKey('is_active');
});
